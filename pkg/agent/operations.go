...
func (a *Agent) Reason(ctx context.Context, prompt string) (string, error) {
	if err := a.transitionTo(agent.StateProcessing); err != nil {
	...
	if err != nil {
		...
		_ = a.transitionTo(agent.StateIdle)
		return "", fmt.Errorf("reason: %w", err)
	}
	...
	ev, err := a.record(event.EventTypeAgentEvaluated.Value(), event.AgentEvaluatedContent{
	...
	if err := a.transitionTo(agent.StateIdle); err != nil {
...
func (a *Agent) Operate(ctx context.Context, workDir, instruction string) (decision.OperateResult, error) {
	...
	// Emit acted event.
	ev, err := a.record(event.EventTypeAgentActed.Value(), event.AgentActedContent{
		AgentID: a.runtime.ID(),
		Action:  "operate",
		Target:  workDir,
	})
...
func (a *Agent) Observe(ctx context.Context, limit int) ([]event.Event, error) {
	// Get events from the shared store — not just this agent's own events.
...
func (a *Agent) Communicate(ctx context.Context, targetID types.ActorID, message string) error {
	ev, err := a.record(event.EventTypeAgentCommunicated.Value(), event.AgentCommunicatedContent{
		AgentID:   a.runtime.ID(),
		Recipient: targetID,
		Channel:   message,
	})
...
// Drives the state machine: Processing → Escalating → Idle.
func (a *Agent) Escalate(ctx context.Context, reason string) error {
	if err := a.transitionTo(agent.StateEscalating); err != nil {
		return fmt.Errorf("escalate: %w", err)
	}

	// Determine who to escalate to from the role hierarchy.
	reportsTo := roles.ReportsTo(a.role)
	targetID := types.MustActorID("human") // default to human
	if reportsTo != "" {
		// In a full system, look up the target agent's actor ID.
	...
	return a.transitionTo(agent.StateIdle)
}

// Refuse declines to perform an action (soul-protected refusal).
// Drives the state machine: Processing → Refusing → Idle.
func (a *Agent) Refuse(ctx context.Context, action, reason string) error {
	if err := a.transitionTo(agent.StateRefusing); err != nil {
