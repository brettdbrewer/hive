...
func (r *Runner) runScout(ctx context.Context) {
	...
	// Gather context.
	stateContext := r.readStateFile()
	gitLog := r.recentGitLog()
	boardSummary := r.boardSummary()

	// Build the scouting prompt.
	prompt := buildScoutPrompt(stateContext, gitLog, boardSummary)
...
func (r *Runner) readStateFile() string {
	if r.cfg.HiveDir == "" {
		return "(state.md not available — HiveDir not set)"
	}
...
func buildScoutPrompt(state, gitLog, board string) string {
...
1. Read the state and identify the SINGLE highest-priority gap.
2. Product gaps outrank code gaps. What would make the product better for users?
3. The task must be CONCRETE and IMPLEMENTABLE in one Operate() call (~3-5 minutes).
4. Don't create tasks that already exist on the board.
5. Don't create vision-level tasks. Be specific: "Add X to Y" not "Design the X system."
...
TASK_DESCRIPTION: <2-3 sentence description with specific files to change>`, state, gitLog, board)
}
