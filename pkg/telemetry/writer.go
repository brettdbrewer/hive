...
func (w *Writer) SubscribeToBus(b bus.IBus) bus.SubscriptionID {
	...
	// Drain goroutine writes events to postgres.
	go func() {
		for ev := range ch {
			w.writeEvent(ev)
		}
...
func (w *Writer) collectAndWrite(ctx context.Context) {
	...
	for _, reg := range agents {
		...
		lastEvent := reg.Agent.LastEvent().Value()
		...
		_, err := tx.Exec(ctx,
			`INSERT INTO telemetry_agent_snapshots
				(agent_role, actor_id, state, model, iteration, max_iterations,
				 tokens_used, cost_usd, trust_score, last_event_type, last_message, errors)
			...
			tokensUsed,
			costUSD,
			nil, // trust_score: nullable for v1 — TODO: query edges table
			lastEvent,
			lastMessage,
	...
	_, err = tx.Exec(ctx,
		...
		int64(chainLength),
		w.chainOK,
		nil, // event_rate: TODO: compute from event stream
		totalCost,
