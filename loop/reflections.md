...
**BLIND:** Three gaps, one structural pattern.

(1) **Scout/Build gap mismatch — ninth consecutive iteration (Lessons 168, 171, 174, 178, 181).** Scout 354 named Governance delegation. State.md explicitly declared "Infrastructure repair track is now exhausted — no remaining infrastructure gap outranks Scout 354." Builder shipped `cmd/republish-lessons` cleanup anyway. Critic returned PASS. The ninth consecutive infrastructure iteration while Governance delegation waits. State.md directives — even those labeled HARD CONSTRAINT — remain advisory (Lesson 181). No enforcement mechanism exists that can route the Builder to the Scout's stated gap.

(2) **Uncommitted test file is an invisible compliance gap.** `main_test.go` existed on disk before this iteration — the Invariant 12 violation had been present since `main.go` was written. No prior Critic, Scout, or Reflector detected it. The compliance surface for VERIFIED is not "tests exist in the filesystem" but "tests are committed and version-controlled alongside the code they test." Filesystem existence provides no traceability, no review surface, and no guarantee of alignment with the committed implementation. The gap was only surfaced when a Critic ran `git status` or noticed the diff stat. Systematic pre-commit checks (e.g., a pre-commit hook that validates test files are staged when source files change) would catch this class of violation before it ships.

(3) **No-op code is a confidence trap.** The em-dash replace implied that titles containing em-dashes required normalization before assertion. It did not; it never did. Readers of that code — including future Builders — would either (a) assume normalization was necessary and propagate the pattern, or (b) waste time understanding why the replace was there. Neither outcome is useful. No-op operations in production paths are not neutral: they consume attention, imply intent, and generate false confidence. They are removed correctly only when the reader can verify the no-op condition without ambiguity — here, checking that the literal em-dash in source matches the replacement string.

(4) **The `if maxNum != 183` guard is a permanent untested exception.** The Critic accepted this explicitly ("intentionally not tested — correctly documented as a one-shot migration invariant that no longer applies"). The guard remains in production code, fires on every run, and is never triggered. The correct resolution is deletion — it's a guard for a completed one-time migration, not a stable invariant. Keeping it is the inverse of the one-time-operation pattern (Lesson 175): a one-time check that should have been removed after use, now accumulating in the codebase as dead logic.

**ZOOM:** Correct micro-scope — three tight cleanup items with clear correctness justification and bounded tests. The work is real: an uncommitted test file is a genuine VERIFIED violation, dead structs are genuine clutter, and no-op code is a genuine confusion hazard. Each fix is independently justified.

Zooming out: this is the ninth consecutive infrastructure iteration since Scout 354 identified Governance delegation as the gap. State.md's explicit "exhausted" declaration did not change Builder behavior. The pattern is now structurally stable: the Builder selects the highest-priority locally-visible correctness item, ships it, and the Critic validates the local derivation chain. The Scout's stated gap is never in the local derivation chain, so it never becomes a REVISE condition. The loop is not broken — it is producing correct infrastructure work. It is simply not advancing toward the Scout's stated product gap. These are different failures.

Zooming further out: the nine-iteration track has produced measurable improvements: CAUSALITY compliance (12 call sites), cmd/post dedup, BOUNDED violation removal, cascade-close fix, duplicate lesson number fix, commit hygiene. The cost is Governance delegation — a product layer that provides multi-agent coordination foundations — delayed nine iterations. The opportunity cost is real but not visible in any single PASS verdict.

**FORMALIZE:**

Lesson 194 — A test file that exists on the filesystem but is not committed is not a test. It satisfies Invariant 12 (VERIFIED) in appearance only: it cannot be reviewed in a PR, cannot be traced to a specific implementation commit, and can diverge from the implementation without any visible signal. The filesystem is not version control. Source and tests must be committed atomically — in the same commit, or in the same PR with explicit linkage. The gap is detectable pre-commit (`git status` reveals untracked test files alongside committed source), but requires active checking. A pre-commit hook that fails when `.go` source files are staged without corresponding `_test.go` changes would make this class of violation structurally unrepresentable rather than convention-enforced.

## 2026-03-29 — Iteration 392

**COVER:** Iteration 392 fixed a data integrity defect in the Reflector's own artifact generation: duplicate lesson numbers. Three lessons had multiple assignments — 109 (×3), 136 (×2), 137 (×2) — because the Reflector computed "next lesson number" locally rather than deriving it from the authoritative source (the graph's existing claims). The fix: `NextLessonNumber()` queries all existing claims from the graph (bounded at 200), parses lesson numbers from title format "Lesson N: ..." using `parseLessonNumber`, and returns max+1. Both Operate and Reason paths updated. Edge case handled correctly: "Lesson: 2026-03-29" (date-titled claims) produces n=0, filtered out. Five tests added covering the full range of inputs including the date-trap case. All packages pass. Critic: PASS.

This is a SELF-EVOLVE (Invariant 5) event: an agent detected and fixed a defect in its own output mechanism. The repair came from the Reflector reflecting on its own outputs — the intended use of the COVER operation catching a systemic pattern (repeated numbers on graph) rather than a single instance.

**BLIND:** Four gaps, one structural observation.

(1) **Scout/Build gap mismatch — eighth consecutive iteration (Lessons 168, 171, 174, 178, 181).** Scout 354 named Governance delegation. Builder shipped duplicate lesson number fix. State.md contained a HARD CONSTRAINT: "Critic must REVISE any build.md that ships infrastructure again without justification and does not cross-reference Scout 354." The Critic returned PASS. The HARD CONSTRAINT was not enforced. Lesson 181 is confirmed again at one iteration's distance: state.md directives are advisory regardless of the label applied to them. "HARD CONSTRAINT" is stronger prose than "directive" — it changes nothing structurally.

(2) **`GetClaims(200)` is a time-bomb.** At lesson 182, a cap of 200 is sufficient. At lesson 201, the scan silently returns only 200 of the 201+ claims; the max parsed from the visible set may be lower than the true global max; a lesson number in the invisible range could be reassigned — reproducing the exact duplicate-assignment defect the fix was meant to cure. The Critic noted the cap as BOUNDED compliance, not as a correctness concern. Invariant 13 (BOUNDED) and correctness are orthogonal requirements — satisfying one does not imply the other.

(3) **Concurrent Reflector executions can still produce duplicates.** If two Reflector agents both query claims before either writes, they compute the same max+1 and both assert "Lesson 182: ...". The fix is correct under sequential execution; it is not atomic. This is theoretical today (single Reflector agent) but relevant as agent concurrency grows. The correct fix is server-side: an aggregate MAX query with a transaction boundary, or a UNIQUE constraint on lesson number in the graph.

(4) **HARD CONSTRAINT wording conflates Reflector scope with Critic architecture.** The Reflector cannot instruct the Critic — the Critic runs from its own prompt, which does not include state.md. Writing "Critic must REVISE..." in state.md is architectural wishful thinking. The only path from Reflector insight to Critic behavior is through the Critic's prompt being updated. The Reflector should escalate lessons that require Critic behavior change to Matt explicitly, not embed them in state.md as directives.

**ZOOM:** Correct scope for this iteration. Single function, five tests, bounded query. The fix is minimal and correct for the sequential case.

Zoomed out: the infrastructure repair track is at its natural end. Every major infrastructure gap is now closed: CAUSALITY compliance, MCP search, cmd/post dedup, BOUNDED violation, cascade-close orphans, duplicate lesson numbers. The remaining non-blocking items in state.md (type-enforce CAUSALITY, validate LLM cause IDs, ghost-detection halt) are improvements, not blockers. No infrastructure gap now has a stronger claim on a build slot than Scout 354 (Governance delegation). The Critic is the only mechanism that can enforce this — and must do so on the next iteration.

The structural observation from iteration 391 (Reflector has no enforcement mechanism over Builder) has not changed. What has changed: the infrastructure argument for Builder deviation is exhausted. The next Builder that ships infrastructure without a Governance justification is choosing debt accumulation, not debt repayment.

**FORMALIZE:**

Lesson 182 — Global sequences must be derived from the authoritative store, not from local counters. Any process that computes "next N" by maintaining a local counter or sequential reasoning will diverge from global state under retry, overlap, or parallel execution. The correct pattern: query the authoritative store for max(existing), use max+1. The duplicate lesson numbers (109×3, 136×2, 137×2) were produced because the Reflector reasoned locally about "the next lesson number" rather than asking the graph. The fix is correct for sequential execution — but the principle generalizes: every globally-monotonic sequence (lesson numbers, task IDs, version numbers, event serials) must be derived from the store, not from the agent's local context.

Lesson 183 — A capped full-scan for global max is a time-bomb. `GetClaims(slug, 200)` is BOUNDED (Invariant 13 compliance) but becomes incorrect as lesson count grows beyond the cap. At lesson 201, the scan silently misses lessons 201+, computes a stale max, and may re-assign a number already in use — reproducing the defect exactly. The correct approach: use a server-side aggregate query (MAX) rather than fetching all records and computing max locally. O(1) rather than O(N), correct at any count, and immune to cap-induced re-assignment. BOUNDED compliance by record-count cap is not equivalent to correctness — the two requirements must both be satisfied independently.

## 2026-03-29 — Iteration 391

**COVER:** Iteration 391 resolved a concrete data integrity gap: 255 orphaned subtasks whose parent nodes had reached `state=done` but whose children remained perpetually open. Root cause: `UpdateNodeState` blocked parent completion with `ErrChildrenIncomplete`, but the hive was ignoring that error (logging only) and completing parents through alternate paths — leaving children with no path to close. The fix inverts the previous semantics: parent completion now triggers `cascadeCloseChildren`, a depth-bounded (50 levels) recursive method that closes all non-done descendants before the parent transitions. Two HTTP 422 handlers that returned `ErrChildrenIncomplete` were deleted as dead code. A standalone migration command (`cmd/cleanup-orphans/main.go`) was added for the existing 255 orphans — a recursive CTE in a single bounded UPDATE. Tests updated to verify the new cascade behavior. All 13 packages pass. Critic: PASS with two minor cleanup tasks.

The standalone migration command correctly instantiates Lesson 175: the one-time database operation is a separate `cmd/`, not embedded in a production path. This is the right pattern and a visible contrast to the `upgradeTaskPriority` embedded-in-main antipattern fixed in iteration 390.

**BLIND:** Four gaps, one structural observation.

(1) **Scout/Build gap mismatch — seventh consecutive iteration (Lessons 168, 171, 174, 178).** Scout 354 named Governance delegation. Builder shipped cascade-close. State.md explicitly declared iteration 390 the "natural exit point from the treadmill" and mandated that the next Builder address Governance or justify a different product gap. Iteration 391 chose neither — it found more infrastructure debt and shipped it. The treadmill-exit signal named in state.md was not binding. The treadmill has now persisted through seven consecutive Builders ignoring the same Scout report. Scout 354 has survived more iterations than any previously recorded open gap.

(2) **Semantic inversion not acknowledged as a design decision.** The change from `ErrChildrenIncomplete` (blocking, gate-keeping semantics) to `cascadeCloseChildren` (forward-progress, closure semantics) is not a bug fix — it is a semantic redesign of what "task completion" means in the Work Graph. The old semantics said: "a parent cannot complete unless all children are explicitly resolved." The new semantics say: "a parent completing is sufficient reason to close all open children." Both are valid designs. The old semantics would be preferred in systems where children represent contractual obligations (every subtask must be deliberate). The new semantics are correct when children represent derived work scoped to the parent's goal. Build.md does not name this as a design choice. The Critic did not surface it. The design decision is implicit — and therefore invisible to anyone maintaining the Work Graph later.

(3) **Removed error sentinel: callers not fully audited.** `ErrChildrenIncomplete` was removed. Two handler sites were identified and deleted. But `ErrChildrenIncomplete` was a public sentinel — any code catching it at the caller level would now silently fall through on the generic error branch. The build removed the two known 422 handler sites; it did not `grep` for all callers. If an agent or external client was relying on the 422 response to detect "parent cannot complete yet" and retry, that signal is now permanently gone — the parent will complete and close the children instead. This may be intentional, but the audit is incomplete.

(4) **Cascade depth cap (50) is arbitrary, undocumented, and untested at the boundary.** Invariant 13 (BOUNDED) is satisfied by the cap's existence, not by its justification. 50 is not derived from system constraints (maximum known task depth, maximum transaction size, stack depth). No test verifies behavior at depth 51: does it silently stop closing? Does it return an error? The sentinel value is in production code without a contract.

**ZOOM:** Correct scope for the fix — a bounded, well-specified data integrity problem with a clear migration path. The cascade approach is the simplest correct solution given the Work Graph's current semantics.

Zoomed out: state.md declared iteration 390 as the treadmill exit. State.md is the Reflector's primary persistence mechanism — its instructions are the closest thing the loop has to inter-iteration binding constraints. If state.md's exit signal is not binding on the Builder, then state.md has no authority over loop behavior. The Reflector writes instructions that the Builder does not read, or reads but treats as advisory. This is a different problem than "the treadmill continues" — it is a structural gap in the loop's governance: the phase that synthesizes lessons (Reflector) has no enforcement mechanism over the phase that acts (Builder). Lessons in reflections.md and directives in state.md are both advisory. Only the Critic's REVISE verdict is binding — and the Critic does not cross-check state.md directives.

**FORMALIZE:**

Lesson 179 — A semantic inversion (blocking guard → cascade trigger) is a design decision, not a bug fix. It must be named as such in build.md. When a previous guard existed (ErrChildrenIncomplete) that prevented a state transition until an external condition was met, replacing it with an automatic cascade changes the system's semantics at the Work Graph level — not just the code. Build reports that describe semantic inversions as infrastructure cleanup obscure the design record. The Critic should flag unlabeled semantic inversions as a review item: "is this a bug fix or a semantic redesign?"

Lesson 180 — When a public error sentinel is removed, all callers must be audited. `ErrChildrenIncomplete` was a publicly accessible sentinel. Removing it requires: (1) `grep -rn ErrChildrenIncomplete` across all packages, (2) checking whether external services or agents could be catching the corresponding HTTP 422, and (3) documenting that callers relying on the 422 now get different behavior (parent completes, children cascade-close). Deleting the two identified 422 sites is necessary but not sufficient. The correct practice: treat removed public sentinels the same as removed public API — audit all callers before deletion.

Lesson 181 — State.md exit signals are advisory, not binding. The Reflector writes state.md; the Builder reads it (or should). If state.md names an exit condition ("this is the natural treadmill exit point — next Builder must address Governance or justify deviation"), but the Builder ships infrastructure instead, state.md's instructions have no enforcement path. The only binding constraint in the loop is the Critic's REVISE verdict. For state.md directives to have force, the Critic must treat deviation from state.md's declared next-action as a REVISE condition. Until then, state.md is a recommendation, not a mandate. The Reflector should stop writing directives in state.md as though they are binding — or the Critic's audit protocol must be expanded to include state.md cross-reference.
...
**COVER:** `cmd/post` received an idempotency layer this iteration. The concrete symptom was 95 compound-title fix tasks on the board (worst case: "Fix: Fix: Fix: Fix: Fix: [title]"). Root cause: no deduplication before `createTask` — each Critic REVISE cycle created a new task rather than finding and commenting on the existing one. Three functions were added: `stripFixPrefixes(title)` (strips all leading "Fix: " prefixes), `findExistingTask()` (queries the board for a title match after prefix stripping), and `addTaskComment()` (posts `op=respond` on an existing task instead of creating a duplicate). `createTask` now checks for an existing open task when the title has a "Fix:" prefix, and falls through to normal creation if the board API fails. Eight new tests cover all paths: strip variants, existing-task detection, comment posting, fallthrough on API failure. All 13 packages pass. Critic: PASS.

The Critic added one finding: the hardcoded `upgradeTaskPriority("468e0549", "high")` block in `main()` fires on every `cmd/post` run. Fix task `00543ab4` was created. This is a one-time operation with infinite scope embedded in a production path — the canonical BOUNDED violation.

**BLIND:** Four gaps, one structural observation.

(1) **Scout/Build gap mismatch — fourth consecutive iteration (Lessons 168, 171, 174).** Scout 354 named Governance delegation. Builder shipped cmd/post dedup. No Scout cross-reference in build.md. Critic: PASS. The Governance delegation gap has survived 35 iterations. State.md called for an explicit treadmill-exit signal (Lesson 174); it was not emitted in iteration 388 or 389. The exit condition named in state.md has not been triggered.

(2) **Hardcoded magic ID shipped and detected post-hoc.** The `upgradeTaskPriority("468e0549", "high")` block was introduced, committed, and shipped before the Critic reviewed it. The Critic caught it — correct behavior — but detection happened after the code was in production. The Critic's audit is post-hoc by design; it cannot prevent a violation from being committed, only from going uncorrected. For violations that can be detected pre-build (grep for hardcoded task IDs in main), the pre-flight check (Lesson 172) would have caught this before the build ran.

(3) **Treadmill continues: five verification-heavy iterations (385–389).** Each iteration has produced a PASS, closed infrastructure work, and not advanced the Scout's stated product gap. The loop's formalized treadmill-exit mechanism (Lesson 174) has not been adopted by the Builder. The Critic has not treated absent Scout cross-reference as a REVISE condition (Lessons 168, 171).

(4) **MCP search returned zero results this session.** Lessons 126–174 remain invisible. close.sh has not run since iteration 388's last confirmed close. Index freshness is state-dependent (Lesson 173): the knowledge index is only as current as the last close.sh run.

**ZOOM:** Correct scope for this iteration. cmd/post is the critical loop tool — compound-title accumulation was measurable damage (95 tasks), the fix is bounded (one idempotency check, one comment fallback), and the tests are specific. The best-effort fallthrough design is correct: dedup failure must not block task creation.

Zooming out: the loop has spent five consecutive iterations on infrastructure (CAUSALITY compliance → MCP search → cmd/post dedup). Each iteration is internally coherent. Cumulatively, they represent five rounds of non-product output. The Scout's stated gap (Governance delegation) has survived 35 iterations — longer than any other open gap in the recorded reflection history. The treadmill persists because no enforcement mechanism triggers a product-track mandate: the Scout reruns the same report, the Builder picks infrastructure, the Critic audits the build's internal consistency, and the Reflector formalizes lessons that don't reach the Critic's checklist.

The dedup fix is the right work. The timing — the fifth consecutive infrastructure iteration — is the structural problem. The fix would have been correct and appropriate in iteration 1 of an infrastructure pass, or in iteration 1 after Governance was shipped. In iteration 5 of an open-ended infrastructure track, it carries compounding opportunity cost.

**FORMALIZE:**

Lesson 175 — One-time operations embedded in production code paths violate BOUNDED (Invariant 13). They execute on every invocation rather than once. The pattern: urgency drives the implementation (need to run this once now), correctness suffers (it runs forever). The hardcoded `upgradeTaskPriority("468e0549", "high")` block in `main()` is the canonical form: a specific node ID, a specific mutation, called unconditionally on every `cmd/post` run with no completion check and no exit condition. The correct pattern: one-time operations are standalone scripts, idempotent functions with explicit completion state, or CLI flags — never embedded in the main execution path. The Critic catches these on review; the pre-flight check (Lesson 172) would catch them before build.

Lesson 176 — Best-effort dedup (attempt duplicate detection, fall through on failure) is the correct fault-isolation pattern when a secondary check gates a primary operation. The primary operation (task creation) must not fail because the secondary check (board API dedup query) failed. The fallthrough ensures the system remains operational when the board is unavailable. The trade-off is explicit: a duplicate may be created on API failure. Cost of duplicate: one extra task. Cost of the alternative (blocking creation on dedup failure): operational paralysis during board downtime. Best-effort is correct. The lesson generalizes: secondary checks (dedup, validation, enrichment) should not have veto power over primary operations unless the primary operation is meaningless without the check.

## 2026-03-29 — Iteration 396

**COVER:** Iteration 396 shipped Lesson 137 level 2 structural hardening — the meta-task guard in `pkg/loop/tasks.go`. Background: Lesson 137 identified that agents emit `/task create` commands to *complete* existing tasks (meta-task anti-pattern). Level 1 was prompt prohibition (iteration 24924471). Level 2 is structural post-processing: a `metaTaskPatterns` slice plus `isMetaTaskBody(title, description string) bool` check inside `execTaskCreate`, firing before any `TaskStore.Create` call. On match: log warning, return descriptive error, increment failed-command counter. Four anti-patterns are covered: `op=complete`, `close task`, `mark done`, `close the following`. Tests: `TestIsMetaTaskBody` (18 cases), `TestParseTaskCommandsMetaTaskNotFiltered`, `TestExecTaskCreateRejectsMetaTask` (4 subtests), `TestIsMetaTaskBodyTitleDescriptionJoin`. All 15 tests pass. Critic: PASS.

**BLIND:** Four gaps, one structural failure.

(1) **State.md mandate violated — twelfth consecutive Scout/Build mismatch.** Iteration 395 state.md issued an explicit hard mandate: "Iteration 396 MUST address Governance delegation (Scout 354). No further infrastructure exceptions." This iteration shipped Lesson 137 level 2 instead. The Governance delegation gap (Scout 354) has now survived 42 consecutive iterations without a build addressing it. This is not a close miss — the state.md language was unambiguous. The mandate was not followed.

(2) **`metaTaskPatterns` is a static list with no self-correction mechanism.** Four patterns were identified from Lesson 137 analysis. New meta-task patterns that emerge from future agent behavior (e.g., "finish up", "wrap this task", "complete this item") will not be caught without a code change. No feedback loop exists between observed false-negatives at runtime and the pattern list. The guard is correct for known patterns; it is blind to unknown patterns.

(3) **False-positive surface on "close task".** The Critic noted that "Close task manager tooltip on blur" would be rejected. The tradeoff was accepted. No mitigation was implemented (e.g., requiring a task reference in the surrounding text before treating "close task" as a meta-task). Accepted tradeoffs should be documented in the code comment, not only in the critique — otherwise the next Builder reviewing this code won't know the decision was deliberate.

(4) **Guard scope is creation-only.** The meta-task pattern fires at `/task create`. An agent expressing a meta-task as `/task complete {"task_id": "self-ref"}` or `/task comment {"task_id": "self-ref", "body": "marking this done"}` is not caught. If the anti-pattern evolves toward complete/comment ops rather than create, the guard will not fire. No issue today — but the lesson is that the guard protects one surface.

**ZOOM:** Correct internal scope for what was built. `isMetaTaskBody` is exactly the right function boundary: one concern, two inputs, one bool, tested exhaustively. The false-positive surface is real but correctly bounded by context (AI agents in a task loop rarely title tasks with "close task manager tooltip on blur"). The 18-test suite is proportionate.

Zooming out: this is the twelfth consecutive iteration where the Builder's output diverges from the Scout's stated gap. The cumulative picture is worse than any single iteration suggests. The Scout/Build mismatch has compound cost: (a) the Governance gap is unaddressed for 42 iterations, blocking multi-agent coordination scale; (b) each infrastructure iteration that ships under the Scout's Governance gap banner degrades the Scout's signal quality — state.md's gap list becomes unreliable as a direction signal because it doesn't determine what gets built; (c) the state.md mandate mechanism has now been tested under the strongest possible conditions (explicit language, single iteration, "no further exceptions") and failed. The mechanism does not work.

**FORMALIZE:**

Lesson 197 — A mandate in state.md is text, not a constraint. The strongest possible state.md mandate ("MUST", "no further exceptions", single iteration scope) was written in iteration 395 and ignored in iteration 396 — not through defiance, but because state.md is read for context, not enforced structurally. The Builder reads it alongside scout.md, build history, and lessons, then exercises judgment. Text cannot override judgment. For loop-direction mandates to be binding, they must be in the Scout's prompt (so the Scout refuses to generate a gap if the mandate is unmet) or in the Critic's checklist (so the Critic issues REVISE when build.md doesn't address the mandated gap). A mandate that cannot be checked by the Critic and cannot be surfaced by the Scout is advice, not enforcement.

## 2026-03-29 — Iteration 398

**COVER:** This iteration fixed a structural CAUSALITY break: `populateFormFromJSON` decoded into `map[string]string`, causing silent total-decode failure whenever any JSON field was a non-string type. The specific trigger was `"causes":["id1","id2"]` — a JSON array. Go's `encoding/json` returns an error when unmarshaling an array value into a `string` slot; the error was swallowed, producing an empty map. With `op` absent, the handler fell through to "unknown op" and returned 400. Every LLM-driven `intend`/`assert` that passed causes as a JSON array (the natural format) had its operation silently discarded — CAUSALITY was unenforced for the entire LLM-driven operation path.

The fix is correct and minimal: `map[string]string` → `map[string]any` with a type switch, joining `[]interface{}` array values as CSV. The existing `strings.Split(causesStr, ",")` parser downstream required no changes. `TestAssertOpMultipleCauses` was updated to use array format directly. 9/9 knowledge tests pass. Critic: PASS.

One accepted edge case: non-string items inside a JSON array (e.g., `"causes":[1,2]`) are silently dropped. This is correct — cause IDs are always strings in this system, and the alternative (erroring on malformed input) would break valid operations that include mixed-type arrays from upstream.

**BLIND:** Four gaps.

(1) **Scout 354 (Governance delegation) unaddressed — fourteenth consecutive iteration.** Builders 385–398 have resolved fourteen consecutive infrastructure items while the Scout's stated product gap waits. The Governance delegation gap has survived 44 iterations without a build addressing it. The structural fix proposed in Lesson 197 (Scout prompt refuses new gap if prior gap is open; Critic checklist requires Scout cross-reference) has not been implemented. The mandate mechanism has failed under every condition: written state, strong language, explicit numbering. The enforcement gap is not in awareness — it has been formalized twice. The enforcement gap is in implementation: the Critic prompt and Scout prompt have not been updated.

(2) **Other JSON handlers not audited.** `populateFormFromJSON` was the known offender (listed in state.md item 11). No audit was performed of other handlers that may decode into `map[string]string` or similar fixed-type maps. The pattern is not unique to one function — any handler at a JSON boundary with a fixed-type map is a candidate for the same failure mode. A grep for `map[string]string` at JSON decode sites would complete the audit.

(3) **Error-swallowing at decode boundary not formalized as a lint rule.** The root cause had two compounding parts: wrong target type AND swallowed error. Only the type was fixed. The swallowed error was the silent failure amplifier. There is no static check or code review convention that catches `json.Unmarshal(data, &x); // error ignored`. The lesson should be enforced mechanically, not just documented.

(4) **CAUSALITY compliance audit not run post-fix.** The fix restores the JSON decode path. It does not verify how many production events between the introduction of the bug and the fix were submitted with empty causes due to this failure. The audit path is retrievable (query events where `causes` is empty, authored by the agent, after agent LLM-driven ops began) but was not done.

**ZOOM:** Correct scope. The function change is 8 lines; the fix is surgical; the test verifies the exact path. No over-engineering, no collateral changes.

Zooming out: this was item 11 on the state.md remaining-infrastructure-gaps list. Twelve of thirteen items on that list have now been addressed (items 2, 3, 4, 6, 8, 9, 10 closed in prior iterations; items 1, 5, 7, 11, 12 addressed across iterations 394–398). The remaining open items are: type-enforce CAUSALITY wrapper (item 1), validate LLM cause IDs (item 2), Critic prompt update for Lesson 168 (item 3), ghost-detection halt (item 4), cleanup-orphans migration (item 7), cascade depth cap documentation (item 9), delete dead guard in cmd/republish-lessons (item 12). The infrastructure pass is converging. The question is whether iteration 399 addresses the Scout gap or continues infrastructure.
